...
describe('HttpClient', () => {
  ...
  describe('makes a POST request', () => {
    it('with text data', done => {
      client.post('/test', 'text body', { observe: 'response', responseType: 'text' }).subscribe(res => {
  ...
  describe('makes a JSONP request', () => {
    it('with properly set method and callback', done => {
      ...
      backend.expectOne({ method: 'JSONP', url: '/test?myCallback=JSONP_CALLBACK' }).flush('hello world')
    })
  })
  describe('makes a request for an error response', () => {
    it('with a JSON body', done => {
      ...
      backend.expectOne('/test').flush({ data: 'hello world' }, { status: HttpStatusCode.InternalServerError, statusText: 'Server error' })
    })
  })
})
